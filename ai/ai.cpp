...
namespace AI
{
    /// @brief AI思考 テトリスAIやビームサーチ型ぷよぷよAIのように多くの探索をせず、22^2で収まっているためこの関数と思考決定は1対1で対応している？
    /// @param field 盤面
    /// @param queue current,next
    /// @param w 重み
    /// @param all_clear 全消し
    /// @param trigger 最大連鎖数がこのtriggerスコアを超えた場合にのみ発火される
    /// @param stretch 連鎖の伸ばし判断の有無
    /// @return AI思考結果
    Result think_1p(
        Field field,
        Cell::Queue queue,
        ...
        bool stretch)
    {
        ...
        // 取りうる連鎖で最もスコアが高いもの（スコアが最も高いもの＝火力が最も高いもの？）
